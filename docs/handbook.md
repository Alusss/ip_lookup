...
| **处理** | 内置监控引擎周期性计算错误率/P99/限流率 → 阈值超过时日志+webhook |
| **输出** | Prometheus 文本格式指标、webhook JSON 告警 |
...
默认 `127.0.0.1:9090/metrics`（仅本地可达），暴露 Prometheus 文本格式：
...
type Monitor struct {
    cfg     *Config
    metrics *Metrics
    alerted *alertState    // cooldown 状态跟踪
    stopCh  chan struct{}
}
```
...
2. 日志 WARN 级别输出
3. 异步 Webhook 推送（支持 generic JSON 和 dingtalk 两种格式）
...
| **metrics 端点** | 默认仅绑定 127.0.0.1:9090 | 保持不暴露公网 |
...
curl localhost:9090/metrics       # → Prometheus 格式
...
| Prometheus 指标 | `127.0.0.1:9090/metrics` | 请求量/限流/状态码/延迟 |
| 自监控告警 | 内置引擎 → Webhook | 错误率/P99/限流率超阈值推送 |

### 告警机制

| 异常类型 | 触发条件 | 通知方式 |
|----------|----------|----------|
| 错误率过高 | 5xx 占比 > 5% | Webhook（generic JSON 或 dingtalk） |
| P99 延迟过高 | P99 > 2000ms | Webhook |
| 限流命中率过高 | > 10% 请求被限流 | Webhook |
